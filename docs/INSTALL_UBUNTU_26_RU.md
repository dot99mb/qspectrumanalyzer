...
## Запуск backend из ярлыка

Запуск приложения полным путём из venv не активирует окружение для поиска
внешних команд. Если `soapy_power` доступен после `source …/bin/activate`, но
не находится при запуске ярлыком, укажите полный путь к нему в **Executable**.
Для окружения из этой инструкции это `$HOME/venvs/qspectrumanalyzer/bin/soapy_power`:
в поле интерфейса замените `$HOME` реальным путём домашнего каталога.
Проверяйте установку запуском этого файла с `--help`, прежде чем менять backend.
