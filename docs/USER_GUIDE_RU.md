...
**2.2.0**. Дата подготовки: **10 сентября 2026 года**. Названия кнопок и полей
...
| CSV recording | Запись частотных отсчётов выше порога в CSV |
...
Во время записи каталог, порог и разделитель заблокированы. Для изменения
этих параметров остановите запись; следующий Start recording создаст новый
...
100300000.0;-45.5;2026-09-10T12:34:56.125+03:00;1250
```

Это пример формата, а не реальное измерение.

| Столбец | Содержимое |
| --- | --- |
| frequency_hz | Частота отсчёта в Hz |
| power_db | Уровень отсчёта в dB |
| system_time | Местные системные дата и время с миллисекундами и часовым поясом |
| elapsed_ms | Целое число миллисекунд от начала текущей записи |
